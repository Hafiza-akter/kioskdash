...
class BanglaDate {
    ...
    function bangla_day( $mewo ) {
        $engNumber = array('Sat', 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri');
        $bangNumber = array('শনিবার', 'রবিবার', 'সোমবার', 'মঙ্গলবার', 'বুধবার', 'বৃহস্পতিবার', 'শুক্রবার ');

        $converted = str_replace( $engNumber, $bangNumber, $mewo );
        return $converted;
    }
    ...
    //     $this->bangDay = $this->bangla_day( $this->bangDay );
    // }
    ...
    // function get_date() {
    //     return array($this->bangDay,$this->bangDate, $this->bangMonth, $this->bangYear);
    // }

        function bangla_Emonth( $mewo ) {
        $eng = array('January', 'February', 'March', 'April', 'May', 'June', 'July','August','September','October','November','December');
        $bang = array('জানুয়ারি', 'ফেব্রুয়ারি', 'মার্চ', 'এপ্রিল', 'মে', 'জুন', 'জুলাই','অগাস্ট','সেপ্টেম্বর','অক্টোবর' ,'নভেম্বর' ,'ডিসেম্বর' );

        $converted = str_replace( $eng, $bang, $mewo );
        return $converted;
    }
    ...
    function convert() {
        $this->bangDate = $this->bangla_number( date("d"));
        $this->bangYear = $this->bangla_number( date("Y") );
        $this->bangDay = $this->bangla_day( date("D") );
        $this->bangla_Emonth = $this->bangla_Emonth( date('F') );
    }
    ...
    function get_date() {
        return array($this->bangDay,$this->bangDate, $this->bangla_Emonth( date('F')), $this->bangYear);
    }
